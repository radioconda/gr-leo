...
tracker::tracker_sptr tracker::make(satellite::satellite_sptr satellite_info,
		const float gs_lat, const float gs_lon, const float gs_alt,
...
tracker::tracker(satellite::satellite_sptr satellite_info, const float gs_lat,
		const float gs_lon, const float gs_alt, const std::string& obs_start,
		const std::string& obs_end, const float time_resolution_us, const std::string& name) :
		d_time_resolution_us(time_resolution_us),
		d_observer(gs_lat, gs_lon, gs_alt), d_tle(
				Tle(satellite_info->get_tle_title(),
						satellite_info->get_tle_1(),
						satellite_info->get_tle_2())), d_sgp4(d_tle), d_obs_start(
				parse_ISO_8601_UTC(obs_start)), d_obs_end(
				parse_ISO_8601_UTC(obs_end)), d_obs_elapsed(d_obs_start) {
...
int tracker::unique_id() {
	return my_id;
}

double tracker::find_max_elevation(Observer& observer, SGP4& sgp4,
...
DateTime tracker::find_crossing_point_time(Observer& observer, SGP4& sgp4,
		const DateTime& initial_time1, const DateTime& initial_time2,
		bool finding_aos) {
	bool running;
	int cnt;
...
double tracker::get_slant_range() {
	double elevation;
...
double tracker::get_current_elevation() {
	 Eci eci = d_sgp4.FindPosition(get_elapsed_time());
	 CoordTopocentric topo = d_observer.GetLookAngle(eci);
	 return topo.elevation;
}

DateTime tracker::parse_ISO_8601_UTC(const std::string& datetime) {
	std::tm tm;
...
void tracker::add_elapsed_time() {
	d_obs_elapsed = d_obs_elapsed.AddTicks(d_time_resolution_us);
}

DateTime tracker::get_elapsed_time() {
	return d_obs_elapsed;
}

bool tracker::is_observation_over() {
